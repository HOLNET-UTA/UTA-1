                          ...
                          const Address &to, NetDevice::PacketType packetType)
{
  ...
  //std::cout << "aaa packet size" << packet->GetSize()<<std::endl;
  ...
  //std::cout << "bbb packet size" << packet->GetSize()<<std::endl;
  Ipv4Header ipHeader;
  ...
  packet->RemoveHeader (ipHeader);
  //std::cout << "ccc packet size" << packet->GetSize()<<std::endl;
  // Trim any residual frame padding from underlying devices
  if (ipHeader.GetPayloadSize () < packet->GetSize ())
    {
      packet->RemoveAtEnd (packet->GetSize () - ipHeader.GetPayloadSize ());
    }
  //std::cout << "dddd packet size" << packet->GetSize() << "ip header get payload " << ipHeader.GetPayloadSize () <<std::endl;
                      ...
                      Ptr<Ipv4Route> route)
{
  ...
  ipHeader = BuildHeader (source, destination, protocol, packet->GetSize (), ttl, tos, mayFragment);
  //std::cout << "packet size in ipv4-l3-protocol" << packet->GetSize() << std::endl;
