...
//                   MakeEnumChecker (CLOSED, "Closed"))
    ...
    .AddAttribute ("Timestamp", "Enable or disable Timestamp option",
                   BooleanValue (false),  //modified by zcw
                   MakeBooleanAccessor (&TcpSocketBase::m_timestampEnabled),
                   MakeBooleanChecker ())
    .AddAttribute ("MaxCWnd", "Max Cwnd size (B)",
                   UintegerValue (180000),  //ADDED by zcw
                   MakeUintegerAccessor (&TcpSocketBase::m_cWndMax),
...
TcpSocketBase::FastRetransmit ()
{
  ...
  NS_LOG_INFO(m_dupAckCount << " dupack. Enter fast recovery mode." <<  //info
...
TcpSocketBase::DupAck ()
{
  ...
  if (m_tcb->m_congState == TcpSocketState::CA_DISORDER)
    {
      //std::cout << "first 3 dupack!!!!" << std::endl;
    ...
    { // Increase cwnd for every additional dupack (RFC2582, sec.3 bullet #3)
      ...
      NS_LOG_INFO (m_dupAckCount << " Dupack received in fast recovery mode." //info
...
TcpSocketBase::ReceivedAck (Ptr<Packet> packet, const TcpHeader& tcpHeader)
{
  ...
  NS_LOG_INFO ("ACK of " << ackNumber <<
                ...
                << " m_recover=" << m_recover); //added by zcw INFO
    ...
    { // Case 3: New ACK, reset m_dupAckCount and update m_txBuffer
      ...
      if (m_tcb->m_congState == TcpSocketState::CA_OPEN)
          ...
          if (ackNumber < m_recover)
            {
              ...
              NS_LOG_INFO ("Partial ACK for seq " << ackNumber <<  //INFO
      ...
      else if (m_tcb->m_congState == TcpSocketState::CA_LOSS)
        {
          ...
          m_dupAckCount = 0;
          m_retransOut = 0;
          NS_LOG_DEBUG ("LOSS -> OPEN, ACKNUMBER: " << ackNumber << " RECOVERNUM" << m_recover);
      ...
      if (callCongestionControl && !(tcpHeader.GetFlags () & TcpHeader::ECE))
        {
          ...
          m_tcb->m_cWnd = std::min(m_tcb->m_cWnd.Get(), m_cWndMax); //added by zcw, limit the max cwnd
          //std::cout << "m_maxCWnd" << m_cWndMax << "m_cwnd" <<  m_tcb->m_cWnd << std::endl; 
        }
...
TcpSocketBase::SendEmptyPacket (uint8_t flags)
{
  ...
  /*added by zcw, for myfifoQueue*/
  if (IsMyFifoQueueDisc ())
    {
        SocketQueueIndexTag queueIndexTag;
        queueIndexTag.SetQueueIndex (GetMyFifoQueueIndex());
        p->AddPacketTag (queueIndexTag);
    }
  /******************************/
  ...
  if (m_retxEvent.IsExpired ())
    {
      ...
      NS_LOG_LOGIC (this << " SendDataPacket Schedule ReTxTimeout at time " <<
                    Simulator::Now ().GetSeconds () << " to expire at time " <<
                    (Simulator::Now () + m_rto.Get ()).GetSeconds () );  //logic
...
TcpSocketBase::SendPendingData (bool withAck)
{
  ...
  while (m_txBuffer->SizeFromSequence (m_tcb->m_nextTxSequence))
    {
      if ((m_ecnState & (ECN_RX_ECHO | ECN_SEND_CWR)) == ECN_RX_ECHO)
        {
          NS_LOG_INFO ("ECE received: decrease ssthresh && cwnd"); //ERROR<-INFO
      ...
      NS_LOG_LOGIC ("TcpSocketBase " << this << " SendPendingData" <<
                    ...
                    " at " << Simulator::Now().GetSeconds()*1000 << "ms");
  ...
  if (nPacketsSent > 0)
    {
      NS_LOG_DEBUG ("SendPendingData sent " << nPacketsSent << " segments, at" << Simulator::Now().GetNanoSeconds() ); //DEBUG
    }
...
TcpSocketBase::EstimateRtt (const TcpHeader& tcpHeader)
{
  ...
  //zcw add
  // std::cout << "h_history, ackeSeq: " << ackSeq <<  "; ";
  ...
  //std::cout << "##m.iszero##" << m.IsZero() << " m: " << m.GetSeconds() << std::endl;
  ...
  if (!m.IsZero ())
    {
      ...
      //std::cout <<  "###rtt###: " << m_rtt->GetEstimate().GetSeconds()*1000 << "ms, at" << Simulator::Now().GetMilliSeconds()  << std::endl;
...
{
  NS_LOG_FUNCTION (this);
  NS_LOG_LOGIC (this << " ReTxTimeout Expired at time " << Simulator::Now ().GetSeconds ()); //logic ZCW
  ...
  if (m_tcb->m_congState != TcpSocketState::CA_LOSS)
    {
      ...
      m_tcb->m_ssThresh = GetSsThresh ();
      m_tcb->m_cWnd = 2*m_tcb->m_segmentSize; //modified by zcw
    }

  m_tcb->m_nextTxSequence = m_txBuffer->HeadSequence (); // Restart from highest Ack
  m_dupAckCount = 0;

  NS_LOG_LOGIC ("RTO. Reset cwnd to " <<  m_tcb->m_cWnd << ", ssthresh to " <<
                m_tcb->m_ssThresh << ", restart from seqnum " << m_tcb->m_nextTxSequence);
  DoRetransmit ();                          // Retransmit the packet zcw
}
...
TcpSocketBase::DoRetransmit ()
{
  ...
  //std::cout << "retransmit seq: " << m_txBuffer->HeadSequence() << std::endl;
